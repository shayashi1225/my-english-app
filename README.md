...
- **スピーキング復習モード** — 日本語の意味だけを見て対応する英語表現をマイクで発話し、AIが語彙選択と発音を評価。結果はSRSのフラッシュカードと同じ間隔反復スケジュールに反映される
...
7. **Review** タブで、過去のセッションに登場した語彙を復習。「📇 フラッシュカード」モードでは意味を見て「もう一度／あいまい／覚えた」で自己採点、「🎤 スピーキング」モードでは日本語の意味だけを見て英語表現をマイクで発話しAIが評価する。ナビゲーションのバッジで復習期日が来た件数が確認できる
